 ...
 */
SDK.bingSpeechTTS = function(utterance, lang, voice) {
	...
	try {
		...
		request.onreadystatechange = function() {
			if (request.readyState != 4) return;
			if (request.status != 200) {
				console.log('Error: Bing Speech web request failed.');
				return;
			}
...
SDK.nativeTTS = function(utterance, lang, voice) {
	...
	if (SDK.responsiveVoice) {
		...
		SDK.bingSpeechTTS(utterance, lang, voice);
		return;
	}
...
SDK.updateAvatar = function(responseMessage, speak, urlprefix, elementPrefix, channelaudio, afterFunction, nativeVoice, lang, voice) {
	...
	if (isVideo && useVideo) {
		...
		if (div != null) {
			...
			if (((SDK.isChrome() && !SDK.isMobile()) || (SDK.isFirefox() && !SDK.isMac()) || SDK.useCanvas == true) && SDK.useCanvas != false && canvasDiv != null) {
...
function SDKConnection() {
	...
	*/
	...
	this.sendObjectDetectionAnalyticImage = function(analyticConfig ,processor, file, form ,image_size) {
		if (!(window.File && window.FileReader && window.FileList && window.Blob)) {
			alert('The File APIs are not fully supported in this browser.');
			return false;
		}
		this.testObjectDetectionAnalytic(analyticConfig, file, true, form, processor, image_size);
	};
	...
	/**
	* refreshAnalytic - TrainingNetwork
	*/
...
function AnalyticObjectDetectionResponse() {
	...
	this.parseXML = function(element) {
		this.parseWebMediumXML(element);
		this.imageBase64 = element.getAttribute("imagebase64");
		this.numberOfClasses = element.getAttribute("numberofclasses");
	}

	this.toXML = function() {
		var xml = "<analytic-object-detection-response";
			if (this.imageBase64 != null) {
				xml = xml + (" imagebase64=\"" + this.imageBase64 + "\"");
			}
			if (this.numberOfClasses != null) {
				xml = xml + (" numberofclasses=\"" + this.numberOfClasses + "\"");
			}
			xml = this.writeWebMediumXML(xml);
...
AnalyticObjectDetectionResponse.prototype = new WebMediumConfig();
